...
export default function Home() {
  const [state, formAction] = useFormState(analyzeCompetitorData, initialState);
    ...
    <div className="flex min-h-screen w-full flex-col bg-background">
      ...
      <main className="flex flex-1 flex-col gap-4 p-4 md:gap-8 md:p-8 lg:flex-row">
        <div className="flex-shrink-0 lg:w-1/3 xl:w-1/4">
          <CompetitorForm action={formAction} />
        </div>
        <div className="flex-1">
          <ResultsDashboard result={state.result} error={state.error} isPending={false} />
        </div>
      </main>
    </div>
  );
